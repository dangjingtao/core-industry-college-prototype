...
test("T047 creates campaigns through the shared dialog with structured form config", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "校园大使计划", exact: true })).toBeVisible();
  ...
  await expect(dialog.getByLabel("活动条款版本").locator("option:checked")).toHaveText("校园大使计划活动条款 · v1.0");
...
test("T047 maintains published terms as frozen versions", async ({ page }) => {
  ...
  const dialog = page.getByRole("dialog", { name: "校园大使计划活动条款" });
  await expect(dialog).toBeVisible();
  await expect(dialog.getByText("校园大使计划活动条款 · v1.0", { exact: true })).toBeVisible();
  ...
  const draft = dialog.locator("div").filter({ hasText: "校园大使计划活动条款 · v2.0" }).filter({ hasText: "草稿 · 可编辑" }).first();
  await expect(draft).toBeVisible();
  await draft.getByRole("button", { name: "编辑草稿" }).click();
  await dialog.getByLabel("条款标题").fill("校园大使计划活动条款");
  await dialog.getByRole("button", { name: "发布并冻结" }).click();

  await expect(dialog.getByText("校园大使计划活动条款 · v2.0", { exact: true })).toBeVisible();
  await expect(dialog.getByText(/已发布并冻结/)).toHaveCount(2);
  await expect(dialog.getByText(/核心大使计划/)).toHaveCount(0);
});
...
test("T047 previews school recruitment QR in a branded dialog", async ({ page }) => {
  ...
  await expect(dialog).toBeVisible();
  await expect(dialog.getByText("校园大使计划 · 校园招募", { exact: true })).toBeVisible();
  await expect(dialog.getByText(/核心大使计划/)).toHaveCount(0);
