  ...
  return <>
    ...
    {adminDomains.map(domain => {
      ...
      return <div key={domain.id}>
        ...
        {!mobile && domain.id === "competitions" && <><NavLink to="/admin/competitions/categories" className={() => subItemClass(location.pathname.startsWith("/admin/competitions/categories"))}><ChevronRight size={13} />比赛分类</NavLink><NavLink to="/admin/competitions/registrations" className={() => subItemClass(location.pathname.startsWith("/admin/competitions/registrations"))}><ChevronRight size={13} />报名记录</NavLink></>}
