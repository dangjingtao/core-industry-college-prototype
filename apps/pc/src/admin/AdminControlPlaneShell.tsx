...
function GlobalNavigation({ mobile = false }: { mobile?: boolean }) {
  ...
  const itemClass = (active: boolean) =>
    `${mobile ? "shrink-0" : "mb-1"} flex min-h-11 items-center gap-3 rounded-control px-3 text-sm font-medium transition ${active ? "bg-surface-subtle text-text-brand" : "text-text-secondary hover:bg-surface-subtle"}`;
