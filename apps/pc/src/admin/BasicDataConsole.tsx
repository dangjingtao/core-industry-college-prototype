...
function sourceBusinessLabel(source: DataSource) {
...
function schoolReviewSummary(organizationId: string) {
  if (organizationId === "org-huanan-commerce-college") return "作为队长学校时，可按具体赛事授权承担团队审核责任。";
  if (organizationId === "org-lingnan-tech-college") return "作为成员学校参与赛事，不因此获得跨校团队的第二审批责任。";
  return "只在已授权赛事和本校范围内处理报名与审核资料。";
}
...
function SchoolDetail({ organizationId }: { organizationId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-2"><div className="rounded-container border border-border-subtle bg-surface p-5"><h3 className="font-semibold">主体说明</h3><p className="mt-3 text-sm leading-6 text-text-secondary">{school.summary}</p><p className="mt-3 text-xs leading-5 text-text-tertiary">{schoolReviewSummary(school.id)}</p><p data-pc05-technical className="mt-2 text-xs leading-5 text-text-tertiary">{school.trust}</p></div><div className="rounded-container border border-border-subtle bg-surface p-5"><h3 className="font-semibold">当前赛事授权 / 关系</h3><div className="mt-3 space-y-2">{school.relations.map(relation => <div key={relation.stableId} className="rounded-control bg-surface-subtle p-3 text-sm"><p className="font-semibold">{relation.label}</p><p data-pc05-technical className="mt-1 text-xs text-text-tertiary">{relation.kind} · {relation.stableId}</p></div>)}</div><p className="mt-3 text-xs text-text-tertiary">赛事授权范围在赛事中心维护，不在学校主体资料里新增认证状态。</p></div></section>
