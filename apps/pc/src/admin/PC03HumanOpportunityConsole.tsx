...
export function PC03HumanOpportunityConsole({ selectedId }: { selectedId?: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {showCreate && (
        <form onSubmit={create} className="rounded-container border border-border-subtle bg-surface p-5 lg:p-6">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <label className="text-xs font-medium text-text-secondary md:col-span-2 xl:col-span-1">机会说明<input name="summary" defaultValue="参与真实业务项目与团队协作。" className="mt-2 min-h-11 w-full rounded-control border border-border px-3 text-sm" /></label>
          </div>
          <div className="mt-5 flex items-center justify-between gap-3"><p className="text-xs text-text-tertiary">创建后默认开放，可随时下架；历史投递不会因下架被删除。</p><button type="submit" className="min-h-11 rounded-control bg-primary-container px-4 text-sm font-semibold text-text-brand">保存并开放</button></div>
        </form>
      )}
