  ...
  return (
    <section data-testid="pc09-hero" className="overflow-hidden rounded-container border border-border-subtle bg-primary text-on-primary">
      <div className="grid gap-6 p-6 lg:grid-cols-[1fr_auto] lg:items-end lg:p-8">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold"><Trophy size={16} />{profile.heroKicker}</div>
          <h1 className="mt-3 text-2xl font-semibold tracking-tight lg:text-3xl">{profile.shortLabel}</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6">独立的是长期赛事运营体验；Competition / Team / Registration 仍复用平台通用真相源。</p>
        ...
        <label className="text-xs font-semibold">运营届次
          <select
            data-testid="pc09-edition-select"
            aria-label="切换三创赛届次"
            value={competitionId}
            onChange={event => navigate(`/admin/sanchuang/${event.target.value}${view === "performance" ? "/performance" : ""}`)}
            className="mt-2 block min-h-11 min-w-64 rounded-control border border-border-subtle bg-surface px-3 text-sm font-semibold text-text-primary"
          >
      ...
      <nav className="flex gap-1 border-t border-primary-pressed bg-primary-pressed px-5 py-3" aria-label="三创赛运营页签">
        <Link to={`/admin/sanchuang/${competitionId}`} className={`rounded-control px-4 py-2 text-sm font-semibold ${view === "overview" ? "bg-surface text-text-primary" : "text-on-primary"}`}>赛事概览</Link>
        <Link to={`/admin/sanchuang/${competitionId}/performance`} className={`rounded-control px-4 py-2 text-sm font-semibold ${view === "performance" ? "bg-surface text-text-primary" : "text-on-primary"}`}>营销实绩</Link>
      </nav>
    </section>
  );
}
...
function OverviewCard({ label, value, detail, icon: Icon }: { label: string; value: string; detail: string; icon: LucideIcon }) {
  return (
    <article className="rounded-container border border-border-subtle bg-surface p-4">
      <div className="flex items-center gap-2 text-xs font-semibold text-text-tertiary"><Icon size={16} className="text-text-brand" />{label}</div>
...
function SanChuangOverview({ competitionId }: { competitionId: string }) {
  ...
  const stages = [...infrastructure.stages].sort((a, b) => a.sort - b.sort);
  ...
  return (
    <div className="space-y-6" data-testid="pc09-overview">
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <OverviewCard label="报名团队" value="1 个稳定样本" detail={`${record.team.name} · 复用 PC02 Team`} icon={UsersRound} />
        <OverviewCard label="当前赛程" value={currentStage?.name ?? "暂无阶段"} detail={currentStage ? `${stageStatusLabel(deriveCompetitionStageStatus(currentStage))} · 复用 PC08 Stage` : "等待赛程配置"} icon={CalendarRange} />
        <OverviewCard label="营销实绩" value={money(summary.gmv)} detail={`${summary.orderCount} 笔订单 · 当前筛选口径`} icon={BarChart3} />
        <OverviewCard label="排名 / 成绩" value="未接评分规则" detail="只预留下钻位，不从营销数据擅自算分" icon={Trophy} />
        <OverviewCard label="获奖结果" value="等待官方回流" detail="外部权威结果到达后再进入可信结果链" icon={PackageCheck} />
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
        <article className="rounded-container border border-border-subtle bg-surface p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold text-text-tertiary">当前运营对象</p>
              <h2 className="mt-1 text-lg font-semibold">{record.name}</h2>
              <p className="mt-2 text-sm text-text-secondary">{record.project.name} · {record.project.track}</p>
            </div>
            <Link to={`/admin/competitions/objects/${competitionId}`} className="inline-flex min-h-10 items-center gap-1 rounded-control bg-primary-container px-3 text-xs font-semibold text-text-brand">通用赛事详情<ChevronRight size={14} /></Link>
          </div>
...
function MetricCard({ testId, label, value, detail, icon: Icon, onClick }: { testId: string; label: string; value: string; detail: string; icon: LucideIcon; onClick: () => void }) {
  return (
    <button type="button" data-testid={testId} onClick={onClick} className="rounded-container border border-border-subtle bg-surface p-4 text-left transition hover:bg-surface-subtle">
      <div className="flex items-center gap-2 text-xs font-semibold text-text-tertiary"><Icon size={16} className="text-text-brand" />{label}</div>
    ...
    <div className="space-y-6" data-testid="pc09-performance">
      <section className="rounded-container border border-border-subtle bg-surface p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <button type="button" data-testid="pc09-export" onClick={exportCurrent} className="inline-flex min-h-11 items-center gap-2 rounded-control bg-primary px-4 text-sm font-semibold text-on-primary"><Download size={16} />导出当前筛选</button>
        </div>
        <div data-testid="pc09-stat-context" className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
      ...
      <section className="grid gap-4 lg:grid-cols-2" data-testid="pc09-source-aggregation">
        {sourceCards.map(card => <article key={card.sourceId} data-testid={`pc09-source-${card.sourceId}`} className="rounded-container border border-border-subtle bg-surface p-5">
      ...
      <section className="rounded-container border border-border-subtle bg-surface" data-testid="pc09-batches">
        ...
        <div className="divide-y divide-border-subtle">{batches.length ? batches.map(batch => <article key={batch.batchId} data-testid={`pc09-batch-${batch.batchId}`} className="grid gap-3 px-5 py-4 lg:grid-cols-[1fr_130px_170px_120px_2fr]">
          <div><p className="text-sm font-semibold">{performanceSourceLabels[batch.source]}</p><p className="mt-1 font-mono text-[11px] text-text-tertiary">{batch.batchId}</p></div>
          <div><p className="text-xs text-text-tertiary">链路</p><p className="mt-1 text-xs font-semibold">{batch.sourceType === "fileImport" ? "导出文件" : "API mock"}</p></div>
          <div><p className="text-xs text-text-tertiary">导入 / 同步时间</p><p className="mt-1 text-xs font-semibold">{batch.importedAt}</p></div>
          <div><span className={`rounded-full px-2 py-1 text-xs font-semibold ${batchStatusClass(batch.status)}`}>{batchStatusLabel(batch.status)}</span><p className="mt-2 text-xs text-text-secondary">{batch.detailCount} 条明细</p></div>
          <p className="text-xs leading-5 text-text-secondary">{batch.note}</p>
        </article>) : <p className="px-5 py-8 text-center text-sm text-text-secondary">当前筛选没有导入 / 同步批次。</p>}</div>
      </section>
      ...
      <section className="rounded-container border border-info bg-info-bg p-4 text-sm leading-6 text-info-text"><strong>评分边界：</strong>当前营销实绩只是第三方数据归集与证据链。原始第三方数据、平台标准字段、聚合展示结果、未来评分结果四层保持分离；在评分规则确认前，本页不会自动计算比赛成绩。</section>
    </div>
  );
}
