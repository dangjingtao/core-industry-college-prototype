  ...
  return <>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 font-semibold">{schoolName} · 大使招募</p>
    ...
    <Dialog
      ...
      title={`${schoolName} · 大使招募二维码`}
      description="用于进入核心大使申请，不用于加入推广团队。"
      size="sm"
      footer={<>
        <SecondaryButton type="button" onClick={() => setOpen(false)}>关闭</SecondaryButton>
        <Button type="button" disabled={!svg} onClick={() => downloadSvg(svg, `${campaignName}-${schoolName}-核心大使招募.svg`)}><Download size={16} className="mr-2" />下载二维码</Button>
      </>}
    >
      <div className="rounded-container bg-primary-container p-5 text-center">
        ...
        <p className="mt-4 text-xs text-text-secondary">扫码进入大使申请入口</p>
...
function CampaignEditorDialog({ campaignId, onDone }: { campaignId?: string; onDone: () => void }) {
  ...
  >
    <div className="space-y-5">
      ...
      <p className="rounded-control bg-info-bg p-3 text-xs text-info-text">点亮门槛固定为 1 位核心大使 + 3 位推广伙伴，不支持自定义规则。</p>
...
function TeamDetail({ campaignId, teamId }: { campaignId: string; teamId: string }) {
  const { campaigns, teams, validAcquisitions, setTeamIncentiveStatus } = useAmbassadorState();
  const campaign = campaigns.find(item => item.id === campaignId);
