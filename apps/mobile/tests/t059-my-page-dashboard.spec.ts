...
test.describe("T059 my page dashboard", () => {
  test("keeps every existing entrance while reorganizing the page as a dashboard", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "我的" })).toHaveCount(0);

    await expect(page.getByRole("link", { name: /张|同学|用户/ }).first()).toBeVisible();
    await expect(page.getByRole("button", { name: "扫一扫" })).toBeVisible();
    await expect(page.getByRole("link", { name: "设置" })).toHaveAttribute("href", "/me/settings");
    ...
    for (const [label, href] of learningLinks) {
      await expect(learningGrid.getByRole("link", { name: label })).toHaveAttribute("href", href);
    }
    ...
    for (const [label, href] of serviceLinks) {
      await expect(serviceGrid.getByRole("link", { name: label })).toHaveAttribute("href", href);
    }
