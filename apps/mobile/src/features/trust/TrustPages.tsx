...
export function CertificateDetailTrustedPage() {
  ...
  return <PublicShell showNavigation={false}><PageHeader title="证书详情" backTo="/assets/certificates" /><div className="space-y-5 px-4 py-5">
    ...
    {isClaimed && <>
      ...
      <Section title="官方平台验真"><Card><p className="text-sm leading-5 text-text-secondary">已签发凭证可前往签发方配置的官方平台继续验真。原型不会伪造一个第三方官方地址。</p><Button className="mt-4 w-full" onClick={() => setHandoffOpened(openOfficialVerificationHandoff(`verificationCode=${item.verificationCode}`))}><ExternalLink size={16} aria-hidden="true" />前往官方验真平台</Button>{handoffOpened && <p className="mt-2 text-xs text-text-tertiary">已打开 handoff 原型页；生产环境替换为签发方真实 URL。</p>}</Card></Section>
    </>}
    <TrustNote />
  </div></PublicShell>;
}
...
export function ResultDetailTrustedPage() {
  ...
  const canDownloadOfficialReport = record.status !== "pending";
  ...
  return <PublicShell showNavigation={false}><PageHeader title="结果详情" backTo="/assets/results" /><div className="space-y-5 px-4 py-5"><Card className="space-y-3"><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-text-secondary">{competition?.name}</p><h1 className="mt-1 text-xl font-semibold text-text-primary">{record.grade}</h1></div><StatusTag tone={statusTone}>{statusLabel}</StatusTag></div>{template && <><p className="text-sm leading-5 text-text-secondary">关联项目成果：{template.title}</p><p className="text-sm leading-5 text-text-secondary">{template.summary}</p></>}</Card>
    {canDownloadOfficialReport ? <Button className="w-full" onClick={() => downloadPrototypeArtifact(`${record.id}-score-report.txt`, [`赛事：${competition?.name ?? record.competitionId}`, `成绩：${record.grade}`, `可信状态：${statusLabel}`, `结果 ID：${record.id}`])}><Download size={16} aria-hidden="true" />下载成绩报告</Button> : <><Button className="w-full" disabled>成绩报告处理中</Button><Card className="border border-warning bg-warning-bg"><p className="text-sm leading-5 text-warning-text">当前结果仍在处理中，不能下载为正式可信成绩报告。阶段成果请在赛事工坊中查看。</p></Card></>}
    {canDownloadOfficialReport && <p className="text-xs leading-5 text-text-tertiary">当前下载为中保真占位文件；真实成绩服务接入后替换为正式报告文件，不改变本页交互。</p>}
...
export function VerificationTrustedPage() {
  ...
  const [handoffOpened, setHandoffOpened] = useState(false);
  const codeRecord = codeChecked ? certificates.find(item => item.verificationCode.toLowerCase() === code.trim().toLowerCase()) : undefined;
  const qrMatch = qrState === "valid" ? certificates.find(item => item.status === "claimed") : undefined;
  const verifiedCredential = codeRecord?.status === "claimed" ? codeRecord : qrMatch;
  ...
  return <PublicShell showNavigation={false}><PageHeader title="可信凭证验真" backTo="/assets" /><div className="space-y-5 px-4 py-5">
    ...
    {mode === "code" && <Card><label className="text-sm font-medium text-text-primary">证书验真码</label><input value={code} onChange={event => { setCode(event.target.value); setCodeChecked(false); }} className="mt-3 min-h-touch w-full rounded-control border border-border bg-surface px-3 text-sm outline-none focus:border-primary" placeholder="输入证书验真码" /><Button className="mt-3 w-full" disabled={!code.trim()} onClick={() => setCodeChecked(true)}>验证</Button>{codeChecked && (codeRecord?.status === "claimed" ? <div className="mt-4 rounded-control border border-success bg-success-bg p-3"><p className="font-semibold text-success-text">验证通过</p><p className="mt-1 text-sm text-success-text">{codeRecord.title} · {codeRecord.issuer}</p><p className="mt-2 text-xs text-success-text">可信事实来自系统证书记录，简历编辑器不可修改。</p></div> : codeRecord?.status === "claimable" ? <div className="mt-4 rounded-control border border-info bg-info-bg p-3"><p className="font-semibold text-info-text">尚未签发</p><p className="mt-1 text-sm text-info-text">该记录当前仅为可领取状态；领取完成后才可作为已签发可信凭证验真。</p></div> : codeRecord?.status === "pending" ? <div className="mt-4 rounded-control border border-warning bg-warning-bg p-3"><p className="font-semibold text-warning-text">待发放 / 处理中</p><p className="mt-1 text-sm text-warning-text">凭证尚未完成签发，当前不能验证为有效可信凭证。</p></div> : codeRecord?.status === "revoked" ? <div className="mt-4 rounded-control border border-danger bg-danger-bg p-3"><p className="font-semibold text-danger-text">凭证已撤销</p><p className="mt-1 text-sm text-danger-text">该验真码对应的凭证已失效。</p></div> : <div className="mt-4 rounded-control border border-danger bg-danger-bg p-3"><p className="font-semibold text-danger-text">未找到有效记录</p><p className="mt-1 text-sm text-danger-text">请检查验真码是否正确。</p></div>)}</Card>}
    ...
    <Section title="官方验证出口"><Card><div className="flex items-start gap-3"><ExternalLink size={20} className="mt-0.5 shrink-0 text-text-brand" aria-hidden="true" /><div><h2 className="font-semibold text-text-primary">前往签发方官方验真平台</h2><p className="mt-1 text-sm leading-5 text-text-secondary">只有已签发并处于有效状态的凭证才开放官方 handoff。生产环境读取签发方配置的真实 URL。</p></div></div><Button className="mt-4 w-full" disabled={!verifiedCredential} onClick={() => verifiedCredential && setHandoffOpened(openOfficialVerificationHandoff(`verificationCode=${verifiedCredential.verificationCode}`))}><ExternalLink size={16} aria-hidden="true" />{verifiedCredential ? "打开官方验真平台（原型）" : "请先验证已签发凭证"}</Button>{handoffOpened && <p className="mt-2 text-xs text-text-tertiary">已打开 handoff 原型页；没有使用虚假的第三方“官方”地址。</p>}</Card></Section>
    <TrustNote />
  </div></PublicShell>;
}
