...
const t013bQuestions: Record<string, DynamicQuestion[]> = {
  "s3-copy-kit": [
    { id: "platform", label: "本次主要发布平台？", type: "single", options: ["抖音", "小红书", "微信视频号"], helper: "选择本次主要运营平台。" },
...
const t013bDynamicTaskIds = new Set(Object.keys(t013bQuestions));
const t013bTaskIds = new Set(["s3-copy-kit", "s3-visual-kit", "s4-weekly-review"]);
...
function outputName(taskId: string, selections: Record<string, string[]> = {}) {
  ...
  if (taskId === "s3-visual-kit") return "图片 / 视频内容生成";
  if (taskId === "s4-weekly-review") return "经营周报分析小报告";
  return "任务成果";
}

function sourceContextTaskId(taskId: string) {
  return taskId === "s3-visual-kit" ? "s3-copy-kit" : taskId;
}
...
function SpecializedAnswerPage() {
  ...
  const firstUnanswered = questions.findIndex(question => !(run?.selections?.[question.id]?.length));
  ...
  if (run && ["queued", "running", "failed"].includes(run.status)) return <SpecializedProgressPage />;
  ...
  const uploadTitle = taskId === "s4-weekly-review" ? "补充经营数据（选填）" : "补充素材（选填）";
  const uploadHint = taskId === "s4-weekly-review" ? "可补 Excel、CSV、截图；不上传也可以继续完成经营周报分析。" : "可补充商品图、品牌素材、PDF 或图片；后续文案与图片 / 视频生成确认共用这份 S3 上下文。";
  ...
  return <PublicShell showNavigation={false}><PageHeader title="动态答题" backTo={`/competitions/${competitionId}/workspace/workshop/skills/${task.skillId}`} /><RequireCompetitionAccess><div className="space-y-6 px-4 py-5">
    ...
    <Card><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-medium text-text-brand">{task.skillId.toUpperCase()} · 动态问答</p><h1 className="mt-2 text-lg font-semibold text-text-primary">{taskId === "s3-copy-kit" ? "平台 / 内容运营" : "经营周报分析"}</h1><p className="mt-2 text-sm leading-5 text-text-secondary">{task.summary}</p></div><StatusTag tone="info">{step + 1}/{questions.length}</StatusTag></div></Card>
    {question && <Section title={`问题 ${step + 1}`} subtitle="按原型逐题回答"><Card data-testid="t013b-dynamic-question"><QuestionField question={question} values={selections[question.id] ?? []} onToggle={toggle} />{currentAnswered && step < questions.length - 1 && <div className="mt-5 rounded-control bg-info-bg p-3" data-testid="t013b-ai-next-feedback"><div className="flex items-center gap-2 text-sm font-medium text-info-text"><Sparkles size={16} aria-hidden="true" />AI 正在分析回答，准备下一题</div></div>}</Card></Section>}
...
function SpecializedReviewPage() {
  ...
  if (!task) return null;
  const runtime = getRuntime(competitionId);
  const sourceTaskId = sourceContextTaskId(taskId);
  const sourceRun = runtime.taskRuns[sourceTaskId];
  const questions = t013bQuestions[sourceTaskId] ?? [];
  const policy = computePolicyForTask(taskId);
  const answered = questions.filter(question => (sourceRun?.selections?.[question.id]?.length ?? 0) > 0).length;
  const completeness = questions.length ? Math.round((answered / questions.length) * 100) : 100;
  const enoughCompute = runtime.computeBalance >= policy.estimateMax;
  const hasSourceContext = completeness === 100;
  ...
  return <PublicShell showNavigation={false}><PageHeader title={isVisual ? "图片 / 视频生成确认" : "生成确认"} backTo={isVisual ? `${taskBasePath(competitionId, "s3-copy-kit")}/answer` : `${taskBasePath(competitionId, taskId)}/answer`} /><RequireCompetitionAccess><div className="space-y-6 px-4 py-5">
    ...
    {taskId === "s3-copy-kit" && <SecondaryButton className="w-full" disabled={!hasSourceContext} onClick={() => navigate(`${taskBasePath(competitionId, "s3-visual-kit")}/review`)}>改为图片 / 视频内容生成</SecondaryButton>}
    <div className="grid grid-cols-2 gap-2"><SecondaryButton onClick={() => navigate(`${taskBasePath(competitionId, sourceTaskId)}/answer`)}>返回修改</SecondaryButton><Button disabled={!ready} onClick={confirm}>确认生成</Button></div>
    <TaskScenarioTools competitionId={competitionId} taskId={taskId} />
  </div></RequireCompetitionAccess></PublicShell>;
}
...
function SpecializedProgressPage() {
  ...
  const runtime = getRuntime(competitionId);
  const run = runtime.taskRuns[taskId];
  const sourceRun = runtime.taskRuns[sourceContextTaskId(taskId)];
  ...
  const generationLabel = taskId === "s3-copy-kit" ? `生成${sourceRun?.selections?.platform?.[0] ?? "平台"}种草图文 / 脚本` : taskId === "s3-visual-kit" ? "生成图片 / 视频示例素材" : "生成经营周报分析小报告";
...
export function T013BTaskAnswerPage() {
  ...
  if (taskId === "s3-visual-kit") return <SpecializedReviewPage />;
  return taskId && t013bDynamicTaskIds.has(taskId) ? <SpecializedAnswerPage /> : <TaskAnswerPage />;
}
