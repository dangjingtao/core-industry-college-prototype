...
export function RegistrationHandoffPage() {
  ...
  useEffect(() => {
    if (!competitionId || !session.loggedIn || !callback || callback.competitionId !== competitionId) return;
  ...
  }, [callback, competitionId, location.pathname, location.search, navigate, session.loggedIn, setCompetitionIdentityScenario, setLifecycle]);
  ...
  const openPortal = () => { if (portalUrl) window.location.assign(portalUrl); };
