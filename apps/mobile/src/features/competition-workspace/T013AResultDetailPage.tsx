...
export function T013AResultDetailPage() {
  ...
  if (!presentation || !competitionId || !resultId) return <LegacyWorkshopResultDetailPage />;
  return <T013AReportPage competitionId={competitionId} resultId={resultId} presentation={presentation} />;
}
...
function T013AReportPage({ competitionId, resultId, presentation }: { competitionId: string; resultId: string; presentation: T013AReportPresentation }) {
  ...
  const initialDraft = result ? runtime.resultDrafts[result.id] ?? emptyDraft(result) : { summary: "", highlights: [], nextSuggestion: "" };
