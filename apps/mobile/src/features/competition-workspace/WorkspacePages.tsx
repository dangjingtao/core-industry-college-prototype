...
export function CompetitionTeamPage() {
  const { competitionId } = useParams();
  const { getRuntime, teamChangeRequestFor, submitTeamChangeRequest } = useWorkshopRuntime();
  ...
  const pendingRequest = teamChangeRequestFor(competitionId);
  const canRequestChange = lifecycle === "inProgress";
  const submitChange = () => {
    if (!memberName || !reason.trim() || !materialName) return;
    submitTeamChangeRequest({ competitionId, type: requestType, memberName, reason: reason.trim(), materialName });
  };
