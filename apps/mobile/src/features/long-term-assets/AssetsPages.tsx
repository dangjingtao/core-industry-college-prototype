...
import { Award, Bell, BriefcaseBusiness, Check, ChevronRight, Download, FileText, GraduationCap, Headphones, HelpCircle, Info, Link2, PenLine, Save, ScanLine, Settings, ShieldCheck, Users, Wallet } from "lucide-react";
...
export function AssetsHomePage() {
  ...
  return <PublicShell><PageHeader title="长期资产" subtitle="比赛会结束，账号和学生资产不会结束" backTo="/me" /><div className="space-y-6 px-4 py-5"><TrustNote />
    ...
    <Section title="下一步"><Card><h2 className="font-semibold text-text-primary">把可信事实整理成自己的履历</h2><p className="mt-2 text-sm leading-5 text-text-secondary">系统事实保持只读；你可以选择哪些经历进入长期简历，并编辑面向岗位的表达。</p><SecondaryButton className="mt-4 w-full" onClick={() => navigate("/me/resume")}>整理长期简历</SecondaryButton></Card></Section></div></PublicShell>;
}
