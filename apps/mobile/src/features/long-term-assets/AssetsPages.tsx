...
import { Award, Bell, ChevronRight, FileText, Headphones, Info, Link2, ShieldCheck } from "lucide-react";
...
export function MyPage() {
  ...
  const accountEntries = [
    { label: "消息通知", description: "查看赛事、投递与平台消息", to: "/me/notifications", icon: Bell },
    { label: "账号绑定", description: "管理邮箱、微信与企业微信", to: "/me/accounts", icon: Link2 },
    { label: "帮助与客服", description: "查找常见问题或联系人工客服", to: "/support", icon: Headphones },
  ];
  const aboutEntries = [
  ...
  return <PublicShell><PageHeader title="我的" subtitle="长期账号资产，不随单场赛事结束" /><div className="space-y-7 px-4 py-5"><Card className="space-y-3"><div><p className="text-xs text-text-secondary">{profile.school} · {profile.major}</p><h1 className="mt-1 text-xl font-semibold text-text-primary">{profile.name}</h1><p className="mt-2 text-sm text-text-secondary">{profile.city} · {profile.email}</p></div><GhostButton className="w-full" onClick={() => navigate("/me/profile")}>编辑基础资料</GhostButton></Card>
    ...
    <Section title="账号与服务"><div className="overflow-hidden rounded-container bg-surface">{accountEntries.map(({ label, description, to, icon: Icon }, index) => <Link key={to} to={to} className={`flex min-h-16 items-center gap-3 px-4 py-3 active:bg-surface-pressed ${index ? "border-t border-border-subtle" : ""}`}><span className="flex size-9 shrink-0 items-center justify-center rounded-control bg-primary-container text-text-brand"><Icon size={18} aria-hidden="true" /></span><span className="min-w-0 flex-1"><strong className="block text-sm font-medium text-text-primary">{label}</strong><span className="mt-0.5 block text-xs leading-5 text-text-secondary">{description}</span></span><ChevronRight size={18} className="shrink-0 text-text-tertiary" aria-hidden="true" /></Link>)}</div></Section>
...
export function AssetsHomePage() {
  ...
  return <PublicShell><PageHeader title="长期资产" subtitle="比赛会结束，账号和学生资产不会结束" /><div className="space-y-6 px-4 py-5"><TrustNote />
    {claimableCertificates.length > 0 && <Section title="待领取"><div className="space-y-3">{claimableCertificates.map(item => {
