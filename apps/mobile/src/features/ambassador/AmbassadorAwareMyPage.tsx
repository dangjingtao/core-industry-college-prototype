...
import { isAmbassadorCodeActive, useAmbassadorState } from "@core/shared";
...
export function AmbassadorAwareMyPage() {
  ...
  const currentTeam = useMemo(() => ambassador.teams.find(team => team.members.some(member => member.accountId === accountId && member.status === "active")), [ambassador.teams, accountId]);
  ...
  const serviceEntries = [
    ...
    ...(currentTeam ? [{ label: "我的团队", to: `/ambassadors/team/${encodeURIComponent(currentTeam.id)}?accountId=${encodeURIComponent(accountId)}`, icon: Users }] : []),
  ...
  <ConfirmDialog open={confirmLogout} title="退出登录" description="只会清除当前登录 session。简历、赛事经历、课程、证书和其它长期账号资产不会被删除。退出后仍可浏览公共平台。" cancelText="取消" confirmText="确认退出" onCancel={() => setConfirmLogout(false)} onConfirm={logout} /></PublicShell>;
}
