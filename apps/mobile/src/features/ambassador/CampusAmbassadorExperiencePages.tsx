...
function TeamRecruitmentQr({ code, teamId }: { code: string; teamId: string }) {
  ...
  return <>
    <Card data-testid="team-recruitment-code" className="overflow-hidden">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 text-sm leading-5 text-text-secondary">分享给校园推荐官扫码加入当前团队。点击二维码可放大，手机端可长按保存。</p>
    ...
    <Dialog open={open} onOpenChange={setOpen} title="团队招募二维码" description="校园推荐官扫码后会直接看到加入确认，不需要输入任何编码。" size="sm">
...
export function CampusAmbassadorLandingPage() {
  ...
  if (code) {
    ...
    return <PublicShell showNavigation={false}><PageHeader title="校园大使计划" backTo="/me" /><div className="px-4 py-6"><Card><h2 className="font-semibold text-text-primary">{message.title}</h2><p className="mt-2 text-sm text-text-secondary">{message.detail}</p></Card></div></PublicShell>;
  }
  return <PublicShell showNavigation={false}><PageHeader title="校园大使计划" backTo="/me" /><div className="px-4 py-6"><Card><div className="flex items-start gap-3"><QrCode className="mt-0.5 text-text-brand" /><div><h2 className="font-semibold text-text-primary">请通过二维码进入</h2><p className="mt-2 text-sm leading-5 text-text-secondary">学校招募码、团队招募码会由扫码器自动识别并进入对应流程，无需手动输入编码。</p></div></div></Card></div></PublicShell>;
}
...
export function CampusAmbassadorApplyPage() {
  ...
  if (!campaign || !schoolCode || !isAmbassadorCodeActive(schoolCode, campaign)) {
    ...
    return <PublicShell showNavigation={false}><PageHeader title="校园大使申请" backTo="/me" /><div className="px-4 py-6"><Card><h2 className="font-semibold text-text-primary">{invalid.title}</h2><p className="mt-2 text-sm text-text-secondary">{invalid.detail}</p></Card></div></PublicShell>;
  }
  if (!session.loggedIn) return <Navigate replace to={`/auth/login?returnTo=${encodeURIComponent(location.pathname + location.search)}`} />;
  if (existingTeam) return <PublicShell showNavigation={false}><PageHeader title="校园大使申请" backTo="/me" /><div className="space-y-4 px-4 py-6"><Card><CheckCircle2 className="text-text-brand" /><h2 className="mt-3 font-semibold text-text-primary">你已加入本期推广团队</h2><p className="mt-2 text-sm text-text-secondary">同一期活动只能属于一个推广团队，不能重复申请校园大使或换队。</p></Card><Button className="w-full" onClick={() => navigate(`/ambassadors/team/${encodeURIComponent(existingTeam.id)}?accountId=${encodeURIComponent(accountId)}`)}>查看我的团队</Button></div></PublicShell>;
  ...
  return <PublicShell showNavigation={false}><PageHeader title="校园大使申请" backTo="/me" /><div className="space-y-4 px-4 py-5">
    <Card className="border border-primary bg-primary-container"><StatusTag tone="info">校园招募</StatusTag><h1 className="mt-3 text-lg font-semibold text-text-primary">申请成为校园大使</h1><p className="mt-2 text-sm text-text-secondary">{campaign.name} · {schoolLabel(schoolId)}</p></Card>
...
export function CampusAmbassadorJoinPage() {
  ...
  const inviter = ambassador?.application?.__applicantName || "校园大使";
  const existingTeam = campaign && state.teams.find(item => item.campaignId === campaign.id && item.members.some(member => member.accountId === accountId && member.status === "active"));
  ...
  return <PublicShell showNavigation={false}><PageHeader title="加入推广团队" backTo="/me" /><div className="space-y-4 px-4 py-5">
    ...
    <Card><div className="space-y-3 text-sm"><div><p className="text-text-tertiary">加入后的身份</p><p className="mt-1 font-medium text-text-primary">校园推荐官</p></div><div><p className="text-text-tertiary">团队规则</p><p className="mt-1 leading-5 text-text-secondary">活动期间不可主动退出或更换团队；团队点亮后你会获得自己的专属推广码。</p></div></div></Card>
