...
export function CampusAmbassadorLandingPage() {
  ...
  if (code) {
    ...
    return <PublicShell showNavigation={false}><PageHeader title="核心大使计划" backTo="/me" /><div className="px-4 py-6"><Card><h2 className="font-semibold text-text-primary">二维码无效或已失效</h2><p className="mt-2 text-sm text-text-secondary">请重新扫描运营方提供的活动二维码。</p></Card></div></PublicShell>;
  }
...
export function CampusAmbassadorApplyPage() {
  ...
  if (!campaign || !schoolCode || !isAmbassadorCodeActive(schoolCode, campaign)) return <PublicShell showNavigation={false}><PageHeader title="核心大使申请" backTo="/me" /><div className="px-4 py-6"><Card><h2 className="font-semibold text-text-primary">当前招募入口不可用</h2><p className="mt-2 text-sm text-text-secondary">活动可能尚未开始、已经结束，或二维码已失效。</p></Card></div></PublicShell>;
...
export function CampusAmbassadorJoinPage() {
  ...
  if (!recruitment || !campaign || !team || !isAmbassadorCodeActive(recruitment, campaign)) return <PublicShell showNavigation={false}><PageHeader title="加入推广团队" backTo="/me" /><div className="px-4 py-6"><Card><h2 className="font-semibold text-text-primary">邀请已失效</h2><p className="mt-2 text-sm text-text-secondary">请联系核心大使重新获取有效的团队招募二维码。</p></Card></div></PublicShell>;
  if (!session.loggedIn) return <Navigate replace to={`/auth/login?returnTo=${encodeURIComponent(location.pathname + location.search)}`} />;
...
export function CampusAmbassadorTeamPage() {
  ...
  return <PublicShell showNavigation={false}><PageHeader title="我的推广团队" backTo={`/me?accountId=${encodeURIComponent(accountId)}`} /><div className="space-y-4 px-4 py-5">
    <Card className={team.status === "lit" ? "border border-primary bg-primary-container" : "border border-warning-border bg-warning-bg"}><div className="flex items-start justify-between gap-3"><div><StatusTag tone={team.status === "lit" ? "success" : "warning"}>{team.status === "lit" ? "已点亮" : "待点亮"}</StatusTag><h1 className="mt-3 text-lg font-semibold text-text-primary">核心大使计划团队</h1><p className="mt-1 text-sm text-text-secondary">{campaign.name}</p></div><Users className="text-text-brand" /></div></Card>
    <Card><div className="flex items-center justify-between"><span className="text-sm text-text-secondary">组队进度</span><strong className="text-text-primary">{memberCount} 人</strong></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-surface-subtle"><div className="h-full rounded-full bg-primary" style={{ width: `${Math.min(100, memberCount / 4 * 100)}%` }} /></div><p className="mt-3 text-xs leading-5 text-text-secondary">点亮条件：1 位核心大使 + 至少 3 位推广伙伴。点亮后仍可继续加入。</p>{remaining > 0 && <p className="mt-2 rounded-control bg-warning-bg px-3 py-2 text-sm text-warning-text">还需 {remaining} 位推广伙伴</p>}</Card>
    {isAmbassador && recruitment && <TeamRecruitmentQr code={recruitment.code} teamId={team.id} />}
    {team.status !== "lit" && <Card className="bg-warning-bg"><h2 className="font-semibold text-warning-text">团队点亮后开放专属推广码</h2><p className="mt-1 text-xs text-text-secondary">当前阶段继续邀请推广伙伴，不会提前产生推广归因。</p></Card>}
    {team.status === "lit" && promotionCode && <PersonalPromotionQr code={promotionCode.code} />}
    {isAmbassador && <Card><div className="flex items-center justify-between"><h2 className="font-semibold text-text-primary">当前成员</h2><span className="text-sm text-text-secondary">{memberCount} 人</span></div><div className="mt-3 divide-y divide-border-subtle">{team.members.filter(item => item.status === "active").map((item, index) => <div key={item.id} data-testid="ambassador-member" className="flex min-h-14 items-center justify-between gap-3"><div><p className="text-sm font-medium text-text-primary">{item.role === "ambassador" ? (item.application?.__applicantName || "核心大使") : `推广伙伴 ${index}`}</p><p className="mt-0.5 text-xs text-text-tertiary">{item.role === "ambassador" ? "核心大使" : "推广伙伴"}</p></div>{item.role === "ambassador" && <StatusTag tone="info">负责人</StatusTag>}</div>)}</div></Card>}
    {isAmbassador && team.status === "lit" && <Button className="w-full" onClick={() => navigate(`/ambassadors/team/${encodeURIComponent(team.id)}/results?accountId=${encodeURIComponent(accountId)}`)}>查看团队推广成果</Button>}
    {!isAmbassador && <Card><div className="flex items-start gap-2"><ShieldCheck size={18} className="mt-0.5 text-text-brand" /><p className="text-sm leading-5 text-text-secondary">推广伙伴只查看团队状态和自己的专属推广码，不展示个人、成员或团队推广成果数字。</p></div></Card>}
  </div></PublicShell>;
}
