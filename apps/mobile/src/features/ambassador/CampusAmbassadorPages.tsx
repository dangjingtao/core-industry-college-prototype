...
function PersonalPromotionQr({ promotionCode }: { promotionCode: string }) {
  const publicSiteUrl = import.meta.env.VITE_PUBLIC_SITE_URL ?? window.location.origin;
  const payload = `${publicSiteUrl}/ambassadors/promote/${encodeURIComponent(promotionCode)}`;
  const [svg, setSvg] = useState("");

  useEffect(() => {
    let cancelled = false;
    QRCode.toString(payload, { type: "svg", errorCorrectionLevel: "M", margin: 1, width: 192 }).then(value => {
      if (!cancelled) setSvg(value);
    });
    return () => { cancelled = true; };
  }, [payload]);

  return <div className="flex justify-center rounded-control bg-white p-3"><div data-testid="personal-promotion-qr" data-payload={payload} data-promotion-code={promotionCode} aria-label={`个人推广二维码 ${promotionCode}`} className="size-48 max-w-full overflow-hidden" dangerouslySetInnerHTML={{ __html: svg }} /></div>;
}
  ...
  return <PublicShell showNavigation={false}>
    ...
    <div className="space-y-5 px-4 py-5">
      <Card className="border border-primary/20 bg-primary-container">
        ...
        <h1 className="mt-3 text-xl font-semibold text-text-primary">成为校园大使，和校园推荐官一起点亮团队</h1>
        <p className="mt-2 text-sm leading-6 text-text-secondary">团队由 1 位校园大使和至少 3 位校园推荐官组成，点亮后才进入推广阶段。</p>
      </Card>
      ...
      </Card> : <Section title="使用学校招募码" subtitle="学校码只用于进入校园大使申请，不会直接加入某个团队">
...
export function CampusAmbassadorApplyPage() {
  ...
  if (!campaign || !recruitment) return <PublicShell showNavigation={false}><PageHeader title="校园大使申请" backTo="/ambassadors" /><div className="px-4 py-6"><Card className="text-center"><p className="font-semibold">招募入口无效</p><p className="mt-2 text-sm text-text-secondary">请使用有效的学校大使招募码。</p></Card></div></PublicShell>;
  if (existingTeam) return <PublicShell showNavigation={false}><PageHeader title="校园大使申请" backTo="/ambassadors" /><div className="space-y-4 px-4 py-6"><Card className="border border-warning/30 bg-warning-bg"><p className="font-semibold text-warning-text">你已绑定本期团队</p><p className="mt-2 text-sm text-warning-text">同一期活动只能属于一个团队，不能重复申请或换队。</p></Card><Button className="w-full" onClick={() => navigate(`/ambassadors/team/${existingTeam.id}?accountId=${encodeURIComponent(accountId)}`)}>查看当前团队</Button></div></PublicShell>;
  ...
  return <PublicShell showNavigation={false}><PageHeader title="申请校园大使" backTo="/ambassadors" /><div className="space-y-5 px-4 py-5">
    <Card><StatusTag tone="info">{campaign.name}</StatusTag><h1 className="mt-3 text-lg font-semibold">校园大使申请</h1><p className="mt-2 text-sm leading-6 text-text-secondary">提交后直接获得“校园大使 · 待点亮”身份，并生成你的团队招募码。</p></Card>
...
export function CampusAmbassadorJoinPage() {
  ...
  return <PublicShell showNavigation={false}><PageHeader title="加入推广团队" backTo="/ambassadors" /><div className="space-y-5 px-4 py-6">
    <Card><UserPlus size={24} className="text-text-brand" /><h1 className="mt-3 text-lg font-semibold">输入团队招募码</h1><p className="mt-2 text-sm leading-6 text-text-secondary">加入后成为校园推荐官，活动期间不能退队或换队。</p></Card>
...
export function CampusAmbassadorResultsPage() {
  ...
  return <PublicShell showNavigation={false}><PageHeader title="团队推广成果" backTo={`/ambassadors/team/${team.id}?accountId=${encodeURIComponent(accountId)}`} /><div className="space-y-5 px-4 py-5">
    <Card className="border border-primary/20 bg-primary-container"><StatusTag tone="success">校园大使可见</StatusTag><h1 className="mt-3 text-xl font-semibold">{campaign.name}</h1><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-control bg-surface p-3"><p className="text-xs text-text-tertiary">团队人数</p><p className="mt-1 text-2xl font-semibold" data-testid="result-member-count">{activeMembers.length}</p></div><div className="rounded-control bg-surface p-3"><p className="text-xs text-text-tertiary">有效新增</p><p className="mt-1 text-2xl font-semibold" data-testid="result-total">{acquisitions.length}</p></div></div></Card>
    <Section title="成员推广成果" subtitle="团队成果仅校园大使可查看"><div className="space-y-2">{activeMembers.map(member => { const count = acquisitions.filter(item => item.promoterAccountId === member.accountId).length; return <Card key={member.id} data-testid="member-result" className="flex items-center justify-between"><div><p className="font-medium">{member.role === "ambassador" ? "校园大使" : "校园推荐官"}</p><p className="mt-1 text-xs text-text-tertiary">账号 {member.accountId}</p></div><strong>{count} 个有效新增</strong></Card>; })}</div></Section>
...
export function CampusAmbassadorPromotionPage() {
  ...
  if (!promotionCode || !campaign || !team || team.status !== "lit" || !codeActive) {
    ...
    return <PublicShell showNavigation={false}><PageHeader title="校园大使专属邀请" backTo="/me" /><div className="px-4 py-6"><Card className="text-center"><p className="font-semibold">{title}</p><p className="mt-2 text-sm text-text-secondary">{detail}</p></Card></div></PublicShell>;
  }
  ...
  const showRegistrationResult = (result: ReturnType<typeof recordPromotionRegistration>) => {
    const messageByResult = {
      ...
      "campaign-member": "该账号已参与本期校园大使推广团队，不计入有效新增",
  ...
  return <PublicShell showNavigation={false}><PageHeader title="校园大使专属邀请" backTo={`/ambassadors/team/${team.id}?accountId=${encodeURIComponent(promotionCode.accountId)}`} /><div className="space-y-5 px-4 py-5">
