...
export type AmbassadorTeam = {
  ...
  /**
   * Internal readable team name used by operations. Optional in the type so
   * persisted V1.0 prototype state remains readable; consumers should use
   * ambassadorTeamDisplayName() when a stable fallback is required.
   */
  teamName?: string;
...
<h3>三、推广码与有效新增</h3>
<p>团队点亮后，校园大使及校园推荐官获得各自的专属推广码。当前活动口径下，仅从未注册过平台的新账号完成注册时计为 1 个有效新增；同一新账号在同一期活动中不重复计算。</p>
<h3>四、推广成果与活动激励</h3>
<p>校园推荐官仅可查看自己的活动身份与专属推广码，不展示个人或团队推广成果数字。校园大使可查看团队推广成果。平台按照活动规则向校园大使处理团队激励，不负责团队成员内部的分配或结算。</p>
<h3>五、活动结束与历史记录</h3>
...
export function ambassadorTeamDisplayName(team: Pick<AmbassadorTeam, "id" | "teamName" | "members">) {
  const explicitName = team.teamName?.trim();
  if (explicitName) return explicitName;
  const ambassadorName = team.members
    .find(member => member.role === "ambassador" && member.status === "active")
    ?.application?.__applicantName?.trim();
  return `${ambassadorName || "校园大使"}团队 · ${ambassadorTeamShortSerial(team.id)}`;
}
