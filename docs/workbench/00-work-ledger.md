...
| T038 | 广告位①：免费福利激励视频 + 权益详情页信息流 | 施工 / 产品决策 | 部分完成（激励视频 + 权益详情页信息流中保真已实现；接入与规则待决策） | T006、T019、T035、F04 Decision A、广告接入 |
| T039 | 广告位②：机会板块信息流（岗位/企业列表第二卡后） | 施工 / 产品决策 | 待确认 | 机会板块（F02/PC03）、广告接入 |
...
- 2026-08-24 实现 T038 广告位①中保真原型：
  ...
  - typecheck + `npm run build:mobile` 通过。
- 状态：待确认 → 部分完成（广告位结构与动线已中保真实现；真实广告 SDK、播放失败降级策略、与 T026 复用协议等仍待决策）。
