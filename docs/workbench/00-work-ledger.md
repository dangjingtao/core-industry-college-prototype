...
| T027 | 邀请码 / 福利码领取入口（新用户邀请码 + 线下活动扫码/填码） | 产品决策 / 施工 | 部分完成（兑换码提示、扫一扫位置已调整；业务模型待决策） | F04 Decision A、T006、T015、T018、扫码/码管理 |
...
| T037 | 智能客服：知识库分流 + AI 工单兜底 | 重设计 / 修订 | 已完成（中保真；双端；设计文档见 docs/product/16、17 与 T037 实现记录） | T009、T009R1、GAP-10 |
| T038 | 广告位①：免费福利激励视频 + 权益详情页信息流 | 施工 / 产品决策 | 已完成（中保真；设计文档见 T038 实现记录） | T006、T019、T035、F04 Decision A、广告接入 |
| T039 | 广告位②：机会板块信息流（岗位/企业列表第二卡后） | 施工 / 产品决策 | 已完成（中保真；高度与列表卡片一致；设计文档见 T039 实现记录） | 机会板块（F02/PC03）、广告接入 |
| T040 | 广告位③：常规 APP 开屏广告 | 施工 / 产品决策 | 已完成（中保真；欢迎页前置一次；设计文档见 T040 实现记录） | T016、广告接入 |
| T041 | 每日打卡入口与新手任务解耦 | 产品决策 / 施工 | 已完成（中保真；任务中心常驻签到条 + 新手任务跳转） | T016、T023、T033、F04 Decision C |
...
| T046 | 核心大使计划 PC 运营工作台 | 施工 | PASS（2026-08-26 Mira 独立复审） | T043、PC01、PC-BD01 |
| T047 | 核心大使计划 PC 运营配置与招募素材收口 | 产品收口 / 施工 | PASS（2026-08-26 Mira 施工复核；PR #18 已合并） | T043、T046 |
| T048 | App 扫码分流、核心大使身份与团队招募体验 | 产品收口 / 施工 | PASS | T043、T044、T045、T047 |
| T049 | 核心大使计划前后台联动、生命周期与最终验收 | 施工 / 收口 / 验收 | PASS（PR #20 已合并；dev Quality Gate 全绿） | T043、T044、T045、T046、T047、T048 |
| T050 | 校园大使 V1.1 展示名与团队名模型 | 产品收口 / 施工 | PASS | T049 |
| T051 | PC 校园大使申请问卷详情视角 | PC 运营 / 问卷详情 / 施工 | PASS（legacy snapshot 与 focused regressions 已复验） | T050 |
| T052 | PC 校园大使周度团队运营与数据导出 | PC 运营分析 / 导出 / 施工 | PASS（UTC+8 自然周；指定团队 / 全部团队 CSV 已复验） | T050 |
| T053 | App 校园大使团队经营视角 | App 经营视角 / 施工 | PASS | T050 |
| T054 | 校园大使 V1.1 前后台联动与最终验收 | 联动 / 收口 / 验收 | PASS（PR #21；Quality Gate + 双端 dev 部署全绿） | T050、T051、T052、T053 |
...
## 补充改动（2026-08-24，随 T038 一并施工）

- 打车券（腾讯地图出行打车券）与外卖券（淘宝闪购无门槛外卖券）从「外部 H5 跳转领取」升级为「API 接口直接发放」：
  - `Benefit` 类型新增 `apiIssued?: boolean` 与 `useInApp?: string`；
  - 两张券 `claimHint` 改为「已通过接口发放到你绑定的手机号，请在「XX」App 内使用相同手机号登录查看/使用」，删除「点击即可跳转 H5 领取」字样；
  - 权益详情页：API 接入券主按钮改为「领取打车券/外卖券」（不再跳 H5），领取成功弹窗提示去对应 App 用相同手机号查看/使用；「领取详情」新增使用方式说明；
  - 非 API 接入的 H5 权益仍保留「直接去合作方页面领取」次级入口。
- 涉及文件：`apps/mobile/src/features/long-term-assets/data.ts`、`BenefitsPages.tsx`。
...
**类型：产品决策卡 / 施工卡**  
**状态：部分完成（兑换码提示、扫一扫位置已调整；业务模型待决策）**  
**优先级：P1**  
...
「需要确认」6 项在中保真范围内的落地取值：

1. 赛道标签由**用户在会话内选择**（含「不确定 / 无赛道」兜底），不做 AI 自动打标；
2. 人工渠道保留三条并存出口：企业微信、站内人工排队、AI 生成站内工单；
3. 工作时间 / 超时阈值本轮不写死数值，改为展示排队快照（等待人数 + 预计时长），阈值待运营确认；
4. 工单状态收敛为四态：待处理 / 处理中 / 已回复 / 已结束（旧候选的「待用户补充」降级为详情页内的补充说明动作，不单独占状态）；
5. 历史工单入口在**帮助中心「我的客服工单」**与**客服浮窗未读跳转**两处，回复红点复用个人中心消息；
6. AI 只承接用户主动填写的问题描述与图片附件，不自动抓取账号敏感字段。

不做：真实 AI 客服系统、SLA 考核、客服绩效、多级客服组织、复杂知识库后台。

实现出口：

- 手机端：`/support`、`/support/chat`、`/support/tickets`、`/support/tickets/:ticketId`；
- PC 端：`/admin/support`、`/admin/support/:ticketId`；
- 实现记录：`docs/workbench/T037-implementation-record.md`。

---

---

# T038｜广告位①：免费福利激励视频 + 权益详情页信息流

**类型：施工卡 + 产品决策卡**  
**状态：待确认**  
**优先级：P1**  
**前置：T006 创赛福利板块、T019 已绑定手机号提示、T035 兑换中心/卡券手机号绑定体验、F04 Decision A、广告接入方案**

## 背景

需要为免费福利领取设计广告位。当前描述：

1. 所有免费福利领取时，点击“领取”后弹出**一段广告**，看完后显示“领取成功”；
2. 免费福利的「权益详情」页下方放置**信息流广告**。
...
- 位置：免费福利「领取」按钮点击后；
- 形式：激励视频 / 插屏激励广告；
- 流程：点击领取 → 播放广告 → 播放完成（或达到可跳过时间）→ 显示“领取成功”；
- 关键：广告观看是领取的前置条件，不是额外奖励。
...
1. 激励视频是否对**所有**免费福利生效，还是仅部分高价值免费福利；
2. 广告长度、是否可跳过、跳过后的领取规则（跳过即放弃领取 vs 看完才能领）；
3. 广告播放失败 / 加载超时 / 广告无填充时的降级策略（直接领取成功？还是提示稍后再试？）——不能因广告故障卡死用户福利；
4. 已绑定手机号用户与未绑定用户是否差异化处理；
5. “领取成功”文案与时机：广告看完即成功，还是广告 + 后续确认步骤；
6. 权益详情页信息流的单页广告条数、刷新策略、与真实内容的混排比例；
7. 广告位是否也适用于付费兑换（T035）或仅免费福利；
8. 学力值 / 权益发放与广告观看的关系（依赖 F04 Decision A 确认学力值模型）；
9. 与 T026 公益助力“看广告得学力值”的关系——是否复用同一广告播放组件/协议；
10. 广告内容审核、行业黑名单、对未成年/学生用户的内容约束。

## 建议验收

- 输出三种状态原型：广告前（领取确认）、广告中（播放/可跳过）、广告后（领取成功）；
- 明确广告失败降级路径，保证免费福利不因广告故障而不可领取；
- 明确信息流广告与真实权益内容的视觉区分，避免用户误点/误判；
- 广告组件与 T026 公益助力可复用同一播放容器与事件埋点。

## 约束

- 免费福利是产品承诺能力，广告只能作为领取激励，**不得成为不可绕过的强障碍导致福利不可达**；
- 广告需有明显标识（如“广告”角标），不得伪装成官方内容或权益；
- 不得在未成年/学生场景投放不适宜广告内容；
- 学力值 / 权益发放规则依赖 F04 Decision A，不自行拍板数值。

## 中保真原型进展

- 2026-08-24 实现 T038 广告位①中保真原型：
  - 新增 `apps/mobile/src/features/long-term-assets/Ads.tsx`：
    - `RewardedVideoAd`：全屏激励视频广告覆盖层（广告位 A），带「广告」标识、倒计时进度、可跳过（3s 后可跳，跳过即放弃本次领取）、到点自动领取成功；
    - `InfoFeedAdCard`：信息流广告卡片（广告位 B），带「广告」标签、广告主、CTA 跳转（原型占位）；
    - 两套示例广告物料 + 按权益 ID 确定性取广告的 `useInfoFeedAd`；
  - `BenefitDetailPage` 权益详情页接入：
    - 免费福利「领取」按钮点击后先弹确认框「观看一段广告后领取成功」，确认后才进入激励视频，看完自动执行领取并显示「领取成功」；
    - 非 API 接入的 H5 权益保留「直接去合作方页面领取」次级入口；
    - 权益详情页底部新增信息流广告卡片；
    - 领取成功弹窗对 API 接入券改为提示去对应 App 用相同手机号查看/使用。
  - typecheck + `npm run build:mobile` 通过。
- 状态：待确认 → 已完成（中保真）。
- 设计文档：`docs/workbench/T038-implementation-record.md`（组件 `apps/mobile/src/features/long-term-assets/Ads.tsx`、接入 `BenefitsPages.tsx`）。

---

# T039｜广告位②：机会板块信息流（岗位/企业列表第二卡后）

**类型：施工卡 + 产品决策卡**  
**状态：已完成（中保真）**  
**优先级：P1**  
**前置：机会板块（F02 企业可信信息 / PC03 机会运营）、广告接入方案**

## 背景

在「机会」板块的列表页插入信息流广告：

- 岗位列表：**第二张卡片之后**插入一条信息流广告；
- 企业列表：**第二张卡片之后**插入一条信息流广告。

## 广告位定义

- 位置：岗位列表 / 企业列表的第 2 张卡片之后；
- 形式：信息流广告卡片，与列表卡片同宽同视觉规格；
- 滚动时随列表自然滚动，不做悬浮或贴片。

## 待确认

1. 是否只在首屏第二卡后插入一条，还是每 N 条卡片循环插入（如每 10 条再插一条）；
2. 岗位列表与企业列表是否使用同一套广告物料规格；
3. 广告卡片与真实岗位/企业卡片的视觉区分方案（角标、底色、间距）；
4. 广告点击跳转目标：H5、外部落地页、还是站内专题；
5. 列表为空 / 只有 1 条卡片时的广告展示规则（不展示？）；
6. 是否按用户画像 / 岗位偏好定向，还是统一投放；
7. 广告曝光 / 点击埋点与归因；
8. 与企业自荐/招聘内容是否冲突（企业本身是资源主体，需避免广告与真实企业信息混淆）。

## 建议验收

- 输出岗位列表 / 企业列表含广告位的列表原型（≥3 条卡片，验证第 2 卡后插入位）；
- 明确广告与真实卡片的视觉边界与“广告”标识；
- 覆盖空列表 / 单卡片 / 多卡片三种列表长度的展示规则。

## 约束

- 广告必须与真实岗位/企业内容明确区分，不得让用户误把广告当成真实机会；
- 企业是赛事/权益/课程/活动/机会的资源与品牌主体，广告不得损害企业可信信息（F02）；
- 列表页广告不改变列表本身的排序逻辑与真实内容优先级。

## 施工记录（2026-08-24）

- 新增 `ListFeedAdCard` 与 `useListFeedAd`（`apps/mobile/src/features/long-term-assets/Ads.tsx`）：
  - 广告卡片与岗位 / 企业列表卡片同构（同 `Card p-3` + `space-y-3` 行结构 / 行高），高度与真实列表卡片一致；
  - 带「广告」角标 + 广告主 + 品牌合作标识 + 查看详情（原型占位跳转）。
- 接入岗位列表（`OpportunitiesPage` positions tab）与企业列表（`OpportunitiesPage` companies tab + `CompaniesPage`）：
  - 在列表第 2 张卡片之后（`index === 1`）插入信息流广告；
  - 列表为空 / 只有 1 张时不展示广告（`index === 1` 不命中）；
  - 广告随列表自然滚动，不做悬浮或贴片。
- typecheck + `npm run build:mobile` 通过。
- 待决策项：是否每 N 条循环插入、按用户画像定向、曝光/点击埋点、真实广告 SDK 接入。
- 设计文档：见 `docs/workbench/T039-implementation-record.md`。

---

# T040｜广告位③：常规 APP 开屏广告

**类型：施工卡 + 产品决策卡**  
**状态：已完成（中保真）**  
**优先级：P1**  
**前置：T016 首页核心功能区重构、广告接入方案**

## 背景

常规 App 开屏广告：

- App 冷启动（或切后台超过阈值后回前台）时展示开屏广告页；
- 展示后进入首页，通常可点击跳转广告落地页或点击“跳过”进入首页。

## 广告位定义

- 位置：App 启动 / 回前台时的全屏开屏层；
- 形式：全屏图片或视频，带品牌落地页跳转入口与“跳过”按钮；
- 倒计时后自动进入首页。

## 待确认

1. 开屏广告的触发时机：冷启动必现？还是热启动 / 回前台也触发（切后台阈值）？
2. 展示时长 / 可跳过时间点（如 3s / 5s 后可跳过）；
3. 无广告填充时的兜底：直接进首页 or 品牌默认图；
4. 点击广告跳转目标与跳转安全校验（外链协议白名单）；
5. 是否做广告频次控制（如每天最多 N 次开屏）；
6. 开屏页与登录 / 免登录首页的关系（未登录用户是否也展示）；
7. 与深链 / handoff 流程的冲突处理：如用户通过注册回流 / 赛事 handoff 进入时是否跳过开屏，避免打断关键动线；
8. 广告素材规范（尺寸、格式、时长）与审核约束。

## 建议验收

- 输出开屏页原型：广告图 + 跳过按钮 + 倒计时 + 落地页跳转；
- 明确冷启动 / 回前台 / 深链进入三种场景的开屏行为；
- 明确无填充 / 加载失败时直接进入首页的兜底路径；
- 开屏广告不得打断注册回流、赛事 handoff、邀请码认领等关键动线。

## 约束

- 开屏广告可跳过、有时限，不得强制无限制观看；
- 不得在登录/认领/回流等关键动线前置广告造成用户流程阻断；
- 广告需带“广告”标识与明确的关闭/跳过入口；
- 对未成年/学生用户执行适宜内容约束。

## 施工记录（2026-08-24）

- 新增 `apps/mobile/src/features/long-term-assets/SplashOverlay.tsx`：
  - `SplashOverlay`：全屏开屏广告层，带「广告」标识 + 倒计时进度条 + 可跳过（3s 后可跳，5s 自动进入）；
  - `useSplashGate`：会话级控制，`sessionStorage` 记录「本会话已看过」，避免原型演示时每次刷新都弹；
...
**状态：待决策**  
**优先级：P1（建议）**  
**前置：T016 首页核心功能区重构、T023 新手任务中心重构、T033 应用中心互动体验 / 轻游戏、F04 Decision C 学力值与任务经济模型**

## 背景

当前每日打卡被封装在「新手任务」内（`/tasks/newbie`）。当用户完成 5 项新手引导后，新手任务页显示「已全部完成」并失去入口，导致：

- 每日打卡没有独立、常驻的入口；
- 用户完成新手任务后难以养成每日回访习惯；
- 与后续可能的连续签到奖励、学力值发放、轻游戏互动（如「我的创业小店」进货）缺少衔接。

## 目标

让「每日打卡」成为与新手任务并行的独立日常入口，满足：

1. 完成新手任务后仍然可打卡；
2. 打卡入口在首页或高频 Tab 可见，路径不超过 2 步；
3. 打卡行为与学力值 / 轻游戏 / 任务中心不产生第二份状态真相源；
4. 原型阶段可模拟连续天数与今日已打卡状态。

## 建议入口方案（可组合）

### 方案 A：首页「任务专区」常驻「每日打卡」卡片（推荐）

- 在首页 `HomeTaskZone` 中，把「每日打卡」作为与「新手任务」「赛事进度」「课程学习」并列的常驻入口；
- 未打卡时显示「今日未打卡 · 点击签到」，已打卡显示「今日已打卡 · 连续 N 天」；
- 优点：路径最短，符合用户每日打开 App 的第一触点；
- 注意：当新手任务未完成时，避免首页入口过多，可将「每日打卡」排在「新手任务」之后或折叠为 secondary。

### 方案 B：任务中心（`/tasks`）顶部增加「今日签到」模块

- 在 `/tasks` 页面顶部加入一个轻量签到条 / 日历组件；
- 展示本月打卡日历、连续天数、今日状态；
- 优点：与「任务中心」语义一致，不增加首页复杂度；
- 注意：`/tasks` 是聚合页，打卡状态仍需读取独立 truth source，不能由任务中心自行产生。

### 方案 C：应用中心增加「每日签到」宫格入口

- 在 `AppCenterPage` 的「学习成长」或「工具与服务」分组中加入「每日签到」；
- 优点：功能入口清晰，与应用中心其他常驻能力并列；
- 注意：应用中心是第 5 Tab，路径比首页深，适合作为补充入口而非唯一入口。

### 方案 D：我的页面顶部展示签到入口

- 在「我的」页面顶部个人信息卡旁边加入「签到」按钮或连续天数；
- 优点：与个人成长、学力值、可信空间自然关联；
- 注意：属于补充入口，单独使用效果较弱。

## 建议最终组合

以 **方案 A（首页常驻）+ 方案 B（任务中心签到条）** 为主，**方案 C（应用中心宫格）** 为辅：
...
1. 每日打卡是否发放学力值 / 积分 / 道具？数量与冷却规则如何定义（F04 Decision C 范围，不得自行拍板）；
2. 连续打卡是否有额外奖励？断签后是否清零或保留历史最高？
3. 打卡是否必须进入 App 内某个页面完成动作（如浏览课程 30 秒），还是「打开 App 即打卡」？
4. 每日打卡与「我的创业小店」「沙盒模拟经营」等轻游戏是否联动（如签到 = 进货 / 补货）？
5. 打卡状态存储位置：本地 `localStorage` 原型模拟，还是后端真实接口？
6. 未登录用户是否允许打卡？如果不允许，点击后引导登录还是隐藏入口？
7. 是否允许补签？补签成本与次数上限？
8. 与通知提醒（T010）联动：未打卡时晚间推送提醒？

## 建议验收

- 输出首页 / 任务中心 / 应用中心三种入口的中保真原型；
- 验证新手任务全部完成后，每日打卡入口仍然可见且可点击；
- 验证已打卡 / 未打卡 / 连续 N 天三种状态展示；
- 验证打卡状态不与新手任务状态耦合；
- 明确打卡奖励规则需等待 F04 Decision C 确认，原型阶段可 placeholder。

## 约束

- 不得把每日打卡并入「新手任务」状态，必须独立存在；
- 不得新建第二份 session / identities / 学力值真相源；
- 打卡奖励、连续签到规则属于 F04 经济模型，施工线程不得自行定义；
- 原型阶段使用本地状态模拟时，需标记为「原型占位」，并预留后端接口替换点。

## 施工记录（2026-08-25）

- 状态：待决策 → 已完成（中保真）。
- 采用方案 B（任务中心常驻签到条）作为 MVP 入口；方案 A/C 作为后续可扩展入口暂不施工。
- 新增 `useCheckIn` 与 `CheckInBar`（`apps/mobile/src/features/task-center/TaskCenterPage.tsx`）：
  - 任务中心顶部常驻签到卡片；
  - 展示「今日已打卡 / 今日未打卡」、连续天数；
  - 点击「打卡」后进入 5 秒激励视频广告全屏层，带倒计时与进度条，可「放弃奖励」；
  - 广告播完后自动打卡成功，弹出「打卡成功，恭喜你获得 20 学力值（暂定）」；
  - 已打卡按钮支持再次点击重置为未打卡，方便演示。
- 状态存储：使用 `localStorage` key `task-center-checkin` 保存 `{ date, streak }`，自然日断签一天以上清零。
- 新手任务联动（`apps/mobile/src/features/public-platform/PublicPlatform.tsx`）：
  - 「每日打卡」任务 `to` 改为 `/tasks`；
  - `useTodayCheckIn` 读取同一 `task-center-checkin` key，任务中心打卡后新手任务自动完成；
  - 移除新手任务内本地打卡动作，统一在任务中心完成。
- typecheck + `npm run build:mobile` 通过。
- 待决策项：是否发放真实学力值、连续奖励规则、补签、与轻游戏联动、未登录态处理、真实后端接口。

---

# T042｜徽章系统（分级徽章 + 可信证书门槛 + 运营大礼包门槛）

**类型：产品决策卡 + 设计卡**  
**状态：待确认（设计文档已产出）**  
**优先级：P1（建议）**  
**前置：F04 Decision A（学力值经济）、D03（任务体系）、T011 / T024（课程体系）、T034（课程考试 / 发证公信力）、T008（可信空间）、T017（长期简历 / 成长档案）**  
**设计文档：`docs/workbench/T042-徽章系统.md`**

## 背景

App 缺乏一个「能持续积累、可展示、有实际作用」的长期成就载体。现有可积累对象分散在课程、权益、证书、赛事成绩里，彼此没有统一呈现与规则。

会议核心判断：**徽章即成就。** 但反方观点成立——单纯成就只有展示意义而无实际意义。因此徽章不能只做荣誉墙，必须挂接实际价值：可信证书门槛、运营大礼包门槛、简历可信能力证据。

## 目标

1. 提供统一、长期、可展示的成就载体（徽章墙）；
2. 徽章分级：低级（APP 行为 / 日常小任务）与高级（课程 / 小测 / 学力值 + APP 行为）；
3. 徽章成为微专业可信证书的组成门槛（必修课程 + 低级 + 高级 + 小测）；
4. 徽章成为运营「大礼包」（大额优惠 / 礼券）的发放门槛，研发只提供基座；
5. 新手任务引导用户完成第一个高级徽章，建立首次正反馈；
6. 徽章作为简历 / 可信空间的可信能力证据（学生自主选择展示）。

## 设计要点（详见设计文档）

- 徽章不强制绑定课程，来源可为 APP 行为 / 课程 / 赛事身份 / 进度 / 小测；
- 可信证书 = 必修课程（更高阶）+ 规定数量低级徽章 + 规定数量高级徽章 + 关卡小测 + 结业线上小考通过；
- 微专业由企业与高校合办、强制学习，课程托管于 App（原则上），作为事实源；
- 高级徽章在固定学习进程发放，领取前须先通过对应关卡小测；
- 课程结束后有结业线上小考，通过后方进入可信证书发放；
- 达成规则透明可解释，不构造不可见评分。
...
| 目的 | 校验该进程知识掌握，领取高级徽章前置 | 校验整体学习成果，可信证书前置 |
| 关系 | 小测通过 → 领取高级徽章 | 小考通过（连同徽章条件）→ 可信证书 |
...
- 学力值发放与徽章奖励关系等 F04 Decision A；
- 徽章与任务体系归属关系等 F04 Decision C；
- 不构造对学生不透明的评分；
